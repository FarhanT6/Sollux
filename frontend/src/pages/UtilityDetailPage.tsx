...
import PaymentSourcePicker, { emptyNewCard, resolvePaymentSource, type NewCardForm } from '../components/payments/PaymentSourcePicker';
...
export default function UtilityDetailPage() {
  ...
  // Which bill(s) the payment goes toward, and a card typed in on the spot.
  ...
  function openPaymentEdit(p: any) {
    ...
    setBill({ statementId: p.statementId ?? '', split: false, alloc: {} });
  ...
  async function savePayment() {
    ...
    try {
      const bankAccountId = await resolvePaymentSource(payForm.bankAccountId, newCard, b => setBankAccounts(prev => [...prev, b]));
      const body = {
        ...
        statementId: bill.statementId || null,
        confirmationNumber: payForm.confirmationNumber || null,
        bankAccountId,
        notes: payForm.notes || null,
      };
      if (editPaymentId) await updatePayment(editPaymentId, body);
      else if (split && 'allocations' in split) await createSplitPayment({ ...body, statementId: undefined, allocations: split.allocations });
      ...
      alert(err?.response?.data?.error ?? err?.message ?? 'Could not save that payment.');
  ...
  useEffect(() => {
    if (!showPayForm || editPaymentId || bill.statementId || bill.split || payForm.amount) return;
    const open = statements.find((st: any) => !(paidMap.get(st.id) ?? false));
    if (!open) return;
    const owed = openBalanceOf(open);
    setBill({ statementId: open.id, split: false, alloc: {} });
    setPayForm(f => ({ ...f, amount: owed != null && owed > 0 ? owed.toFixed(2) : f.amount }));
        ...
        {tab === 'payments' && (showPayForm || editPaymentId) && (
          <div className="rounded-xl px-5 py-4 mb-3" style={{ background: '#1e1e1e', border: '1px solid rgba(245,166,35,0.25)' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              ...
              <BillPicker className="sm:col-span-2" statements={statements} payments={payments}
                amount={parseFloat(payForm.amount) || 0} value={bill} onChange={setBill} />
              <PaymentSourcePicker accounts={bankAccounts} paymentMethod={payForm.paymentMethod}
                value={payForm.bankAccountId} onChange={id => setPayForm(f => ({ ...f, bankAccountId: id }))}
